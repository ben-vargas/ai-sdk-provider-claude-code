...
- **New SDK 0.3.x option passthroughs in `ClaudeCodeSettings`** - `skills` (`string[] | 'all'` single-switch skills enablement; no need to add `'Skill'` to `allowedTools`), `settings` (inline `Settings` object or settings file path), `managedSettings` (restrictive policy-tier settings), `toolAliases` (map built-in tool names to replacement tools), `toolConfig` (per-tool configuration), `planModeInstructions`, `title`, `forwardSubagentText`, `agentProgressSummaries`, and `includeHookEvents`. A runnable `examples/skills-option.ts` (`npm run example:skills-option`) demonstrates the `skills` setting end-to-end with a self-contained temp-project skill — a positive run with no `allowedTools` wiring, and a `skills: []` contrast showing omitted skills are rejected at invocation (enablement allowlist, not a sandbox).
...
- **`permissionDenials` in `providerMetadata['claude-code']`** - Auto-denied tool calls (`permission_denied` messages) are warn-logged and recorded as `{ toolName, toolUseId?, reason? }` entries, merged (deduped by `tool_use_id`) with the result message's `permission_denials` list — the only place PreToolUse-hook denials surface — so denials are visible without waiting for the model to mention them.
- **`onPromptSuggestion` callback in `ClaudeCodeSettings`** - Receives the predicted next user prompt when `promptSuggestions: true`. The suggestion arrives after the `result` message (post-finish), which is why it is delivered via callback instead of `providerMetadata`. The post-result drain is bounded: it stops after the first suggestion (the SDK emits at most one per turn) and times out after 10s so a lingering CLI cannot hold the subprocess open indefinitely. A runnable `examples/prompt-suggestions.ts` (`npm run example:prompt-suggestions`) bridges the post-finish callback with a promise + bounded race and feeds the suggestion back as the next turn.
...
- **Context usage documentation (3.x, superseded in 4.x)** - Initially documented how to call `query.getContextUsage()` via the raw `onQueryCreated` callback and a `Stop` hook while the CLI subprocess is live. In the 4.x line, the README and `examples/context-usage.ts` use `onQueryControllerCreated` / `ClaudeCodeQueryController.getContextUsage()` for the same timing requirement, with `controller.rawQuery` still available for raw SDK access.
...
- **`createAiSdkMcpServer(name, tools)` helper** - Bridges AI SDK tool definitions (the `ai` package's `tool()` helper) into an in-process SDK MCP server for the `mcpServers` setting. Automatic bridging via the AI SDK `tools` option is impossible at the provider layer (providers only receive tool declarations; `execute` lives in the `ai` package layer), so this is the explicit alternative. String results pass through as MCP text content, other results are `JSON.stringify`'d, and thrown errors become `isError` tool results instead of crashing the CLI session. Requires a Zod object schema and an `execute` function on every tool — `jsonSchema()`-based or execute-less tools throw at creation time. The Agent SDK's `tool()` performs FIELD-LEVEL validation and transforms and strips unknown keys before `execute`; object-level constructs (`.refine()`/`.superRefine()`, `.strict()`/`.passthrough()`/`.catchall()`) are NOT enforced by the bridge (re-parsing would re-run transforms and reject valid transform schemas), so do cross-field/unknown-key checks inside `execute`. Also exports the structural `AiSdkLikeTool`/`AiSdkToolExecuteOptions` types (no dependency on the `ai` package), adds a "Using AI SDK Tools" README section, and ships a runnable `examples/ai-sdk-tools.ts` (`npm run example:ai-sdk-tools`).
- **Provider-executed tool parts in `doGenerate` content** - `doGenerate` now emits `tool-call`/`tool-result` content parts for CLI-executed tools, interleaved with text/reasoning parts in arrival order, so `generateText` steps content carries the tool information that previously existed only on the streaming path (matching field shapes: `providerExecuted`/`dynamic` flags, `rawInput`/`rawResult`/`rawError` plus `parentToolCallId` in `providerMetadata['claude-code']`, Task-subagent parent tracking, and `maxToolResultSize` truncation). CLI `tool_error` blocks surface as `tool-result` parts with `isError: true`: the V3 content union has no `tool-error` member and AI SDK core's `asContent()` drops unknown content part types, so an `isError` tool-result is the shape that round-trips into a proper `tool-error` part in `generateText` steps content (the streaming path keeps its provider-extension `tool-error` stream part, which AI SDK core handles natively). The block-to-part conversion is shared between `doGenerate` and `doStream` so the two paths cannot drift, and refusal-fallback `supersedes` retractions drop tool parts from superseded assistant messages alongside their text and thinking — including cleaning up parent-tracking state (a retracted `Task` no longer pollutes fallback parent inference) and dropping late tool results for retracted calls instead of emitting orphans.
- **Warm-start re-exports (`startup` / `WarmQuery`)** - The SDK's `startup()` helper pre-spawns the CLI subprocess and completes its initialize handshake, returning a `WarmQuery` handle whose `query()` writes the prompt to the ready process (no startup latency). Note the documented limitation: a `WarmQuery` is a standalone SDK query path (its `query()` returns the SDK `Query` directly, once per handle) and the SDK exposes no option for handing a pre-warmed process to a regular `query()` call, so warm start cannot accelerate `generateText`/`streamText` requests made through this provider. The README section "Reducing time-to-first-token (warm start)" documents the standalone usage pattern, and a runnable `examples/warm-start.ts` (`npm run example:warm-start`) compares a cold `generateText` baseline against the warm path side-by-side using the new timing metadata (`ttftMs`, `ttftStreamMs`, `timeToRequestMs`, `warmSpareClaimed`).
...
- **Permission hooks example** - `examples/hooks-permission-denied.ts` (`npm run example:hooks-permissions`) demonstrates a `PreToolUse` hook that returns `permissionDecision: 'allow'` for known-safe tools and NO decision otherwise (deferring to the permission system / `canUseTool`), plus a `PermissionDenied` hook observing tools auto-denied without a prompt. It deliberately does NOT return `permissionDecision: 'defer'` alongside `canUseTool` — that combination currently fails the tool call before `canUseTool` is consulted (see the upstream caveats above).
...
- **3.x README SDK-boundary section** - Documented the then-unmapped `Options` fields (`agent`, `onElicitation`), the provider-managed fields, and why the SDK's alpha `/browser`, `/bridge`, and `/assistant` entry points were not re-exported. In the 4.x line, `agent` and `onElicitation` are first-class settings; only provider-managed fields and alternate SDK entry points remain boundaries.
...
- **`hooks`/`canUseTool` upstream caveats documented** - TSDoc and README now document two verified CLI 2.1.172 behaviors: a `PreToolUse` hook returning `permissionDecision: 'defer'` combined with `canUseTool` fails the tool call before `canUseTool` is consulted, and the `PermissionDenied` hook only fires for auto-mode classifier denials (`canUseTool` denies surface via `providerMetadata['claude-code'].permissionDenials` instead).
...
- **JSON Schema `format` keywords no longer silently kill structured output** - Schemas containing `format` (`date-time`, `email`, `uri`, `uuid`, ... — produced by Zod's `.datetime()`, `.email()`, `.url()`, `.uuid()`) made the Claude Code CLI return a successful result WITHOUT `structured_output` (verified on SDK 0.2.63 and 0.3.170 — upstream CLI limitation), surfacing downstream as an opaque `AI_NoObjectGeneratedError`. The provider now deep-walks the `outputFormat` schema (`properties`, `items`/`prefixItems`, `anyOf`/`oneOf`/`allOf`, `not`, `$defs`/`definitions`, `additionalProperties`, `patternProperties`, ...) and strips every `format` keyword, folding the hint into the field's `description` (e.g. `(expected format: email)`); stripped paths are debug-logged. This is safe end-to-end because `generateObject`/`streamObject` validate against the original Zod schema client-side. `pattern` is deliberately untouched (the CLI genuinely enforces simple regex patterns; lookaheads/backreferences remain a documented limitation). `examples/zod4-compatibility-test.ts` uses the real format validators again.
- **Missing `structured_output` now fails loudly (with graceful recovery)** - When structured output was requested but the CLI result carries no `structured_output` (and none was streamed), both `doGenerate` and `doStream` first try to parse the prose response as JSON (the final assistant turn's trimmed text, then fenced ` ``` ` blocks last-to-first, then — in `doGenerate` — all turns' prose joined) and use it as the object text with a warning; only JSON objects/arrays count as recovered (scalar prose like `null` or `42` does not). If recovery fails, a descriptive non-retryable `APICallError` pointing at the README structured-output limitations is thrown instead of letting prose flow into an opaque `AI_NoObjectGeneratedError`. The existing `error_max_structured_output_retries` and truncation behaviors are preserved.
