...
async function main() {
  ...
  // This example writes to REAL session storage (~/.claude/projects/), so
  // every created session is registered here and removed in the finally
  // block even when a later step fails.
  ...
  try {
    ...
    createdSessionIds.push(fork.sessionId);
    ...
    // Deletion happens in the finally block below so sessions are removed
    // even if any earlier step throws; here we just verify the lookup result
    // after cleanup runs.
  } finally {
    for (const id of createdSessionIds.reverse()) {
      try {
        ...
        console.warn(`Cleanup: could not delete session ${id}:`, cleanupError);
      }
    }
    if (createdSessionIds.length > 0) {
      const afterDelete = await getSessionInfo(createdSessionIds[createdSessionIds.length - 1]);
