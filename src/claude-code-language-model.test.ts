...
describe('ClaudeCodeLanguageModel', () => {
  ...
  describe('guarded rewind reuse', () => {
    ...
    describe.each(['doGenerate', 'doStream'] as const)('%s failures', (method) => {
      it.each(['init-then-throw', 'init-then-abort', 'error-result', 'error-subtype'] as const)(
        ...
        async (failure) => {
          ...
          vi.mocked(mockQuery)
            .mockReturnValueOnce({
              async *[Symbol.asyncIterator]() {
                ...
                if (failure === 'init-then-abort') throw new MockAbortError('Request aborted');
          ...
          await expect(invoke(instance, method)).rejects.toThrow(
            failure === 'init-then-abort' ? /abort/i : 'Resume rejected by --resume-drops-turn:'
          );
      ...
      it('does not consume a non-success result even when is_error is false', async () => {
        const target = {
          resume: 'source-session',
          resumeSessionAt: 'kept-message',
          resumeDropsTurn: 'discarded-prompt',
    ...
    describe.each(['doGenerate', 'doStream'] as const)('%s SDK delivery options', (method) => {
      const cases: Array<{
        ...
        { name: 'unset defaults', settings: {} },
        {
          ...
          settings: { permissionPrompts: 'host', pluginDelivery: 'argv' },
          permissionPrompts: 'host',
          pluginDelivery: 'argv',
        },
        {
          name: 'headless prompts and stdin plugins',
          settings: { permissionPrompts: 'none', pluginDelivery: 'initialize' },
        ...
        },
        {
          ...
          settings: {
            permissionPrompts: 'host',
            pluginDelivery: 'argv',
            sdkOptions: { permissionPrompts: 'none', pluginDelivery: 'initialize' },
          },
          permissionPrompts: 'none',
          pluginDelivery: 'initialize',
        },
        {
          ...
          settings: {
            permissionPrompts: 'none',
            pluginDelivery: 'initialize',
            sdkOptions: { permissionPrompts: undefined, pluginDelivery: undefined },
          },
          permissionPrompts: 'none',
          pluginDelivery: 'initialize',
        },
      ...
      it.each(cases)('preserves $name at the SDK boundary', async (testCase) => {
        ...
        const call = vi.mocked(mockQuery).mock.calls[0]?.[0];
        for (const key of ['permissionPrompts', 'pluginDelivery'] as const) {
