...
| `permissionPrompts`               | Who answers permission prompts: `'host'` (default) uses `canUseTool` or `permissionPromptToolName`; `'none'` denies requests that would prompt and never calls `canUseTool`. Permission mode, rules and hooks still apply.                                                                                                                                              |
| `pluginDelivery`                  | How plugin paths reach Claude Code: `'argv'` (default) uses flags; `'initialize'` sends them over stdin to avoid command-line length limits. Requires Claude Code 2.1.261+; the bundled binary qualifies.                                                                                                                                                               |
...
For hosts without an approval UI, set `permissionPrompts: 'none'`. Permission mode, rules and hooks still decide whether a tool can run; requests that would otherwise prompt are denied immediately, and `canUseTool` is never called. Omit this option (or use `'host'`) to route prompts to `canUseTool` or `permissionPromptToolName`. The SDK still rejects combining those two handlers, including when prompts are disabled.
