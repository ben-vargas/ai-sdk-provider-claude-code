...
| `permissionMode`                  | Permission mode: `'default'`, `'acceptEdits'`, `'bypassPermissions'`, `'plan'`, `'dontAsk'`, `'auto'` (`'auto'` added in SDK 0.3.x; `'delegate'` was dropped from the SDK type but remains accepted by this provider — both in the `ClaudeCodeSettings` type and at runtime — for backward compatibility) |
